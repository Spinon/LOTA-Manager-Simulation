...
function buildBlackboard(
  ...
  return {
    ...
    controlReservations: (previous?.controlReservations ?? []).filter((reservation) => reservation.expectedEnd > time),
    damageReservations: (previous?.damageReservations ?? []).filter((reservation) => reservation.expectedImpactTime + 0.8 > time),
    saveReservations: (previous?.saveReservations ?? []).filter((reservation) => reservation.expectedImpactTime + 0.8 > time),
    formationPlan: previous?.formationPlan ? {
      ...previous.formationPlan,
      anchorPosition: { ...previous.formationPlan.anchorPosition },
      frontlineHeroIds: [...previous.formationPlan.frontlineHeroIds],
      midlineHeroIds: [...previous.formationPlan.midlineHeroIds],
